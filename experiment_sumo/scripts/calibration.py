...
def main():
    ...
    lines += [
        ...
        "reference (reproduced by scaling the pedestrian vType `maxSpeed` by "
        "`SPEED_CALIB` to offset JuPedSim's ~0.914x free-flow factor), and "
        "speed decreases monotonically with density as expected for JuPedSim's "
        "CollisionFreeSpeedModel.  Over-capacity cells (narrow × high demand) "
        "are detected via the density / flow-stability floor and assigned "
        "`q_r* = 0` (plan Case 1).",
        "",
        "> Note: JuPedSim's CollisionFreeSpeedModel reduces every agent to a "
        "scalar collision disc (`radius = max(length, width)/2`), so the robot "
        "enters as a 0.48 m disc (Camello 0.96 × 0.70 m) rather than a true "
        "oriented rectangle. The PRE (Pedestrian Replacement Equivalent) "
        "analysis quantifies how many pedestrians that disc is worth as a "
        "function of width, flow, and split.",
        "",
    ]
