...
def fig_pre_surface():
    path = os.path.join(PATHS["sensitivity"], "pre_surface.csv")
    if not os.path.exists(path):
        print("(PRE figure skipped: run pre.py first)")
        return
    df = pd.read_csv(path)
    piv = df.pivot(index="qp", columns="W", values="PRE")
    ...
    ax.set_xlabel("Effective width W (m)")
    ax.set_ylabel("Pedestrian flow q_p (ped/min)")
    ax.set_title("Pedestrian Replacement Equivalent PRE (1 robot ≈ X ped)")
    med = np.nanmedian(piv.to_numpy())
    for i in range(piv.shape[0]):
        for j in range(piv.shape[1]):
            v = piv.to_numpy()[i, j]
            if np.isfinite(v):
                ax.text(j, i, f"{v:.1f}", ha="center", va="center",
                        color="white" if v > med else "black", fontsize=8)
    fig.colorbar(im, ax=ax, label="PRE")
    fig.savefig(os.path.join(FIG, "fig_pre_surface.png"))
    plt.close(fig)
