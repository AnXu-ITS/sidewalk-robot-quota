...
def case1_cells(baseline_df, dfloor=None, outmin=None, outmax=None):
    """Return the set of (W, qp) whose pedestrian-only state is unacceptable.

    Over-capacity signals (mean over seeds):
      * mean_density > dfloor                     (over-crowding)
      * mean_flow_ratio < outmin                  (throughput collapse)
      * mean_flow_ratio > outmax                  (spawn-blocking jam; SUMO
        blocks spawns at capacity so flow_ratio >> 1)
    """
    dfloor = CONSTRAINTS["density_floor"] if dfloor is None else dfloor
    outmin = 0.85 if outmin is None else outmin
    outmax = CONSTRAINTS["outflow_ratio_max"] if outmax is None else outmax
    bad = set()
    for (W, qp), g in baseline_df.groupby(["W", "qp"], sort=True):
        mean_density = g["mean_density"].mean()
        mean_flow_ratio = g["flow_ratio"].mean()
        if mean_density > dfloor or mean_flow_ratio < outmin or \
                mean_flow_ratio > outmax:
            bad.add((W, qp))
    return bad
...
def derive_quota(baseline_df, sweep_df, delta_v=None, conf=None):
    ...
    for W in WIDTHS_TRAIN:
        for qp in PED_FLOWS_TRAIN:
            ...
            base_pass = (mean_density_base <= dfloor) and flow_ok(mean_fr_base)
