...
from config import (SIM, CONSTRAINTS, PATHS, WIDTHS_TEST, PED_FLOWS_TEST,
                    ROBOT_FLOWS, ROBOT_FLOWS_ALL, N_SEEDS_VALIDATION)
...
def load_model():
    ...
    C_knots = None
    if "C_W" in P:
        C_knots = (np.asarray(P["C_W"]["W"], float),
                   np.asarray(P["C_W"]["C"], float))
    q_max = P.get("q_max", None)
    return x_crit, W_min, A, B, iso, C_knots, q_max
...
def derive_test_quota(baseline_df, sweep_df, cells):
    ...
    for W, qp in cells:
        ...
        base_pass = (base["mean_density"].mean() <= dfloor and
                     flow_ok(base["flow_ratio"].mean())) if len(base) else False
        qr_star = 0.0
        above = False
        if base_pass:
            ...
            for qr in [q for q in ROBOT_FLOWS_ALL if q != 0]:
                ...
                pass_mean = (mean_Rv >= (1.0 - delta_v) and
                             flow_ok(mean_fr) and mean_rho <= dfloor)
                passes[qr] = pass_mean
                if pass_mean:
                    qr_star = qr
...
def main(n_procs=None):
    ...
    sweep_scen = [(W, L, qp, qr, s, None, None, None)
                  for W, qp in cells
                  for qr in ROBOT_FLOWS_ALL if qr != 0
    ...
    for key, name, params in [("A", "Model A", A), ("B", "Model B", B),
        ...
        pred = predict_quota(qp, W, key, params, x_crit, W_min, C_knots, q_max)
