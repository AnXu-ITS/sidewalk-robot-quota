...
DATA = os.path.join(HERE, "cells")
OSM_DIR = os.path.join(HERE, "osm")
CELLS_JSON = os.path.join(DATA, "selected_cells.json")
OUT_EST = os.path.join(DATA, "site_flow_estimate.csv")
OUT_CTX = os.path.join(DATA, "context_coverage.md")

CITY_OSM = {"nyc": "nyc", "amsterdam": "amsterdam", "melbourne": "melbourne",
            "taipei": "taipei", "newtaipei": "newtaipei", "seattle": "seattle",
            "taoyuan": "taoyuan"}
...
def main():
    ...
    for c in cells:
        ...
        if poi is None:
            ...
            continue
        # coverage guard: does the POI file contain ANY node within 200 m?
        n_near = (sum(1 for m in poi["mrt"] if hav(lat, lon, m[0], m[1]) <= 200)
                  + sum(1 for s in poi["shop"] if hav(lat, lon, s[0], s[1]) <= 200)
                  + sum(1 for a in poi["amen"] if hav(lat, lon, a[0], a[1]) <= 200)
                  + sum(1 for bl, bo, _t in poi["landuse"]
                        if hav(lat, lon, bl, bo) <= 200))
        cov_n[city] += 1
        if n_near == 0:
            cov[city] += 1
    ...
    print("POI coverage guard (cells with NO poi within 200 m):",
          dict(cov), "/", dict(cov_n))
