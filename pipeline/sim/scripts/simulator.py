...
def run_scenario(W, L, qp, qr, seed, PED, WALL, ROBOT, SIM,
    ...
    try:
        ...
        with open(log, "w", encoding="utf-8") as lf:
            try:
                ...
                lf.write(f"\n[timeout] killed after "
                         f"{SIM.get('timeout_sec', 300.0)} s\n")
        if timed_out:
            # conservative: a hung scenario counts as a FAILED measurement
            return {
                "W": float(W), "L": float(L), "qp": float(qp), "qr": float(qr),
                "seed": int(seed), "v_ref": float(v_ref),
                "r_robot": float(r_robot),
                "mean_speed": 0.0, "speed_count": 0,
                "mean_density": 99.0,
                "inflow_ped": 0, "outflow_ped": 0,
        ...
        metrics = {
            ...
            "flow_ratio": float(flow_ratio), "timed_out": 0,
        }
        return metrics
    finally:
